  ...
  )
{
  ...
  Status = SpdmProtocol->SendReceiveSessionData (SpdmProtocol, SessionId, &Request, RequestSize, &Response, &ResponseSize);
  ASSERT_EFI_ERROR(Status);
  ...
  )
{
  ...
  *(UINT32 *)Parameter.AdditionalData = SessionId;
  for (Index = 0; Index < ARRAY_SIZE(mDataTypeString); Index++) {
    ...
    Status = SpdmProtocol->GetData (SpdmProtocol, mDataTypeString[Index].DataType, &Parameter, Data, &DataSize);
    if (!EFI_ERROR(Status)) {
      Print (L"%a (%d) - ", mDataTypeString[Index].String, DataSize);
      ...
      Status = SpdmTestProtocol->GetData (SpdmTestProtocol, mDataTypeString[Index].DataType, &Parameter, TestData, &TestDataSize);
      ASSERT_EFI_ERROR(Status);
      if (DataSize != TestDataSize) {
        Print (L"-- Verify DataSize FAIL, ");
      } else {
        Print (L"-- Verify DataSize PASS, ");
      }
      if (CompareMem(Data, TestData, DataSize) != 0) {
        Print (L"Verify Data FAIL\n");
      } else {
        Print (L"Verify Data PASS\n");
      }
